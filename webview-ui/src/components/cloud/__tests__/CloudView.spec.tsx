	...
	useAppTranslation: () => ({
		t: (key: string) => {
			const translations: Record<string, string> = {
				...
				"cloud:signIn": "Connect to ACode Cloud",
				"cloud:cloudBenefitsTitle": "Connect to ACode Cloud",
				...
				"cloud:visitCloudWebsite": "Visit ACode Cloud",
				"cloud:taskSync": "Task sync",
				"cloud:taskSyncDescription": "Sync your tasks for viewing and sharing on ACode Cloud",
				...
				"cloud:remoteControlDescription":
					"Enable following and interacting with tasks in this workspace with ACode Cloud",
				...
				"cloud:cloudUrlPillLabel": "ACode Cloud URL: ",
			}
...
describe("CloudView", () => {
	it("should display benefits when user is not authenticated", () => {
		...
		expect(screen.getByRole("heading", { name: "Connect to ACode Cloud" })).toBeInTheDocument()
	...
	it("should display remote control toggle when user has extension bridge enabled and roomote control enabled", () => {
		...
		expect(
			screen.getByText("Enable following and interacting with tasks in this workspace with ACode Cloud"),
		).toBeInTheDocument()
	})
	...
	it("should not display cloud URL pill when pointing to production", () => {
		...
		expect(screen.queryByText(/ACode Cloud URL:/)).not.toBeInTheDocument()
	})
	...
	it("should display cloud URL pill when pointing to non-production environment", () => {
		...
		expect(screen.getByText(/ACode Cloud URL:/)).toBeInTheDocument()
