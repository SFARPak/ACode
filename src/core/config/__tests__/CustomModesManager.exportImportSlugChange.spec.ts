...
import type { ModeConfig } from "@acode/types"
...
describe("CustomModesManager - Export/Import with Slug Changes", () => {
	...
	const mockRoomodes = path.join(mockWorkspacePath, ".acodemodes")
	...
	describe("Import with Slug Changes", () => {
		it("should place files in rules-{new-slug} folder when slug is changed", async () => {
			...
			const rule1Path = Object.keys(writtenFiles).find(
				(p) => p.includes("rule1.md") && !p.includes(".acodemodes"),
			)
			const rule2Path = Object.keys(writtenFiles).find(
				(p) => p.includes("rule2.md") && !p.includes(".acodemodes"),
			)
			...
			expect(rule1Path).toContain(path.join(".acode", "rules-new-slug-name", "rule1.md"))
			expect(rule2Path).toContain(path.join(".acode", "rules-new-slug-name", "subfolder", "rule2.md"))
		...
		it("should handle mixed format paths correctly", async () => {
			...
			expect(oldFormatPath).toContain(path.join(".acode", "rules-mixed-mode", "old-format.md"))
			expect(newFormatPath).toContain(path.join(".acode", "rules-mixed-mode", "new-format.md"))
			expect(nestedPath).toContain(path.join(".acode", "rules-mixed-mode", "nested", "file.md"))
		})
	})
	...
	describe("End-to-End Export/Import with Slug Change", () => {
		it("should successfully export and re-import with a different slug", async () => {
			...
			const ruleFilePath = Object.keys(writtenFiles).find(
				(p) => p.includes("rule.md") && !p.includes(".acodemodes"),
			)
			expect(ruleFilePath).toBeDefined()
			expect(ruleFilePath).toContain(path.join(".acode", "rules-renamed-mode", "rule.md"))
