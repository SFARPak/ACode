...
describe("RooIgnoreController", () => {
	...
	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .acodeignore exists
		 */
		it("should load .acodeignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .acodeignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".acodeignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".acodeignore"), "utf8")
		...
		/**
		 * Tests the controller behavior when .acodeignore doesn't exist
		 */
		it("should allow all access when .acodeignore doesn't exist", async () => {
			// Setup mocks to simulate missing .acodeignore file
		 ...
		 */
		it("should set up file watcher for .acodeignore changes", async () => {
		 ...
		 */
		it("should handle errors when loading .acodeignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .acodeignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		...
		/**
		 * Tests the default behavior when no .acodeignore exists
		 */
		it("should allow all access when no .acodeignore content", async () => {
			// Create a new controller with no .acodeignore
			mockFileExists.mockResolvedValue(false)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .acodeignore exists
		 */
		it("should allow all commands when no .acodeignore exists", async () => {
			// Create a new controller with no .acodeignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new RooIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .acodeignore
		 */
		it("should generate formatted instructions when .acodeignore exists", async () => {
			// Setup .acodeignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
		 ...
		 * Tests behavior when no .acodeignore exists
		 */
		it("should return undefined when no .acodeignore exists", async () => {
			// Setup no .acodeignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .acodeignore is created
		 */
		it("should reload .acodeignore when file is created", async () => {
			// Setup initial state without .acodeignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .acodeignore
			controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .acodeignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .acodeignore is changed
		 */
		it("should reload .acodeignore when file is changed", async () => {
			// Setup initial state with .acodeignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .acodeignore is deleted
		 */
		it("should reset when .acodeignore is deleted", async () => {
			// Setup initial state with .acodeignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
