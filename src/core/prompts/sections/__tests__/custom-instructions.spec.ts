...
describe("loadRuleFiles", () => {
	...
	it("should not combine content from multiple rule files when they exist", async () => {
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			if (filePath.toString().endsWith(".acoderules")) {
	...
	it("should use .acode/rules/ directory when it exists and has files", async () => {
		// Simulate .acode/rules directory exists
		...
		readdirMock.mockResolvedValueOnce([
			{
				...
				parentPath: "/fake/path/.acode/rules",
			},
			{
				name: "file2.txt",
				isFile: () => true,
		...
		statMock.mockImplementation((path) => {
			...
			if (
				normalizedPath.includes("/fake/path/.acode/rules/file1.txt") ||
				normalizedPath.includes("/fake/path/.acode/rules/file2.txt")
			) {
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.acode/rules/file1.txt") {
				return Promise.resolve("content of file1")
			}
			if (normalizedPath === "/fake/path/.acode/rules/file2.txt") {
		...
		const expectedPath1 =
			process.platform === "win32"
				? "\\fake\\path\\.acode\\rules\\file1.txt"
				: "/fake/path/.acode/rules/file1.txt"
		const expectedPath2 =
			process.platform === "win32"
				? "\\fake\\path\\.acode\\rules\\file2.txt"
				: "/fake/path/.acode/rules/file2.txt"
		...
		const expectedRulesDir =
			process.platform === "win32" ? "\\fake\\path\\.acode\\rules" : "/fake/path/.acode/rules"
	...
	it("should filter out cache files from .acode/rules/ directory", async () => {
		// Simulate .acode/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.acode/rules/rule1.txt") {
				return Promise.resolve("rule 1 content")
			}
			if (normalizedPath === "/fake/path/.acode/rules/rule2.md") {
			...
			if (normalizedPath === "/fake/path/.acode/rules/.DS_Store") {
				return Promise.resolve("DS_STORE_BINARY_CONTENT")
			}
			if (normalizedPath === "/fake/path/.acode/rules/Thumbs.db") {
				return Promise.resolve("THUMBS_DB_CONTENT")
			}
			if (normalizedPath === "/fake/path/.acode/rules/backup.bak") {
				return Promise.resolve("BACKUP_CONTENT")
			}
			if (normalizedPath === "/fake/path/.acode/rules/cache.log") {
				return Promise.resolve("LOG_CONTENT")
			}
			if (normalizedPath === "/fake/path/.acode/rules/temp.tmp") {
				return Promise.resolve("TEMP_CONTENT")
			}
			if (normalizedPath === "/fake/path/.acode/rules/script.pyc") {
		...
		const expectedCacheFiles = [
			"/fake/path/.acode/rules/.DS_Store",
			"/fake/path/.acode/rules/Thumbs.db",
			"/fake/path/.acode/rules/backup.bak",
			"/fake/path/.acode/rules/cache.log",
			"/fake/path/.acode/rules/temp.tmp",
			"/fake/path/.acode/rules/script.pyc",
		]
	...
	it("should fall back to .acoderules when .acode/rules/ is empty", async () => {
		// Simulate .acode/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
	...
	it("should read files from nested subdirectories in .acode/rules/", async () => {
		// Simulate .acode/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readdirMock.mockResolvedValueOnce([
			...
			},
			{
				...
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.acode/rules/subdir",
			},
			{
				name: "nested2.txt",
				isFile: () => true,
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.acode/rules/subdir/subdir2",
			},
		] as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.acode/rules/root.txt") {
				return Promise.resolve("root file content")
			}
			if (normalizedPath === "/fake/path/.acode/rules/subdir/nested1.txt") {
				return Promise.resolve("nested file 1 content")
			}
			if (normalizedPath === "/fake/path/.acode/rules/subdir/subdir2/nested2.txt") {
		...
		const expectedRootPath =
			process.platform === "win32" ? "\\fake\\path\\.acode\\rules\\root.txt" : "/fake/path/.acode/rules/root.txt"
		const expectedNested1Path =
			process.platform === "win32"
				? "\\fake\\path\\.acode\\rules\\subdir\\nested1.txt"
				: "/fake/path/.acode/rules/subdir/nested1.txt"
		const expectedNested2Path =
			process.platform === "win32"
				? "\\fake\\path\\.acode\\rules\\subdir\\subdir2\\nested2.txt"
				: "/fake/path/.acode/rules/subdir/subdir2/nested2.txt"
...
describe("addCustomInstructions", () => {
	...
	it("should combine all instruction types when provided", async () => {
		// Simulate no .acode/rules-test-mode directory
		statMock.mockRejectedValueOnce({ code: "ENOENT" })
		...
		expect(result).toContain("Rules from .acoderules-test-mode:\nmode specific rules")
	})
	...
	it("should include AGENTS.md content along with other rules", async () => {
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (pathStr.endsWith(".acoderules")) {
				return Promise.resolve("Roo rules content")
			}
			return Promise.reject({ code: "ENOENT" })
		})
		...
		// Should contain both AGENTS.md and .acoderules content
		expect(result).toContain("# Agent Rules Standard (AGENTS.md):")
		expect(result).toContain("Agent rules content")
		expect(result).toContain("# Rules from .acoderules:")
	...
	it("should use .acode/rules-test-mode/ directory when it exists and has files", async () => {
		// Simulate .acode/rules-test-mode directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readdirMock.mockResolvedValueOnce([
			{
				...
				isFile: () => true,
				isSymbolicLink: () => false,
				parentPath: "/fake/path/.acode/rules-test-mode",
			},
			{
				name: "rule2.txt",
				isFile: () => true,
		...
		statMock.mockImplementation((path) => {
			...
			const normalizedPath = path.toString().replace(/\\/g, "/")
			if (
				normalizedPath.includes("/fake/path/.acode/rules-test-mode/rule1.txt") ||
				normalizedPath.includes("/fake/path/.acode/rules-test-mode/rule2.txt")
			) {
				return Promise.resolve({
					isFile: vi.fn().mockReturnValue(true),
				}) as any
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.acode/rules-test-mode/rule1.txt") {
				return Promise.resolve("mode specific rule 1")
			}
			if (normalizedPath === "/fake/path/.acode/rules-test-mode/rule2.txt") {
		...
		const expectedTestModeDir =
			process.platform === "win32" ? "\\fake\\path\\.acode\\rules-test-mode" : "/fake/path/.acode/rules-test-mode"
		const expectedRule1Path =
			process.platform === "win32"
				? "\\fake\\path\\.acode\\rules-test-mode\\rule1.txt"
				: "/fake/path/.acode/rules-test-mode/rule1.txt"
		const expectedRule2Path =
			process.platform === "win32"
				? "\\fake\\path\\.acode\\rules-test-mode\\rule2.txt"
				: "/fake/path/.acode/rules-test-mode/rule2.txt"
	...
	it("should fall back to .acoderules-test-mode when .acode/rules-test-mode/ does not exist", async () => {
		// Simulate .acode/rules-test-mode directory does not exist
		statMock.mockRejectedValueOnce({ code: "ENOENT" })

		// Simulate .acoderules-test-mode exists
		readFileMock.mockImplementation((filePath: PathLike) => {
			if (filePath.toString().includes(".acoderules-test-mode")) {
	...
	it("should fall back to .clinerules-test-mode when .acode/rules-test-mode/ and .acoderules-test-mode do not exist", async () => {
		// Simulate .acode/rules-test-mode directory does not exist
		statMock.mockRejectedValueOnce({ code: "ENOENT" })
	...
	it("should correctly format content from directories when using .acode/rules-test-mode/", async () => {
		...
		readdirMock.mockResolvedValueOnce([
			{ name: "rule1.txt", isFile: () => true, parentPath: "/fake/path/.acode/rules-test-mode" },
		] as any)
		readFileMock.mockReset()
...
describe("Rules directory reading", () => {
	it.skipIf(process.platform === "win32")("should follow symbolic links in the rules directory", async () => {
			...
			.mockResolvedValueOnce([
				{
					...
					isFile: () => true,
					parentPath: "/fake/path/.acode/rules/symlink-target-dir",
				},
			] as any)
		...
		statMock.mockImplementation((path: string) => {
			// For directory check
			if (path === "/fake/path/.acode/rules" || path.endsWith("dir")) {
				return Promise.resolve({
					isDirectory: vi.fn().mockReturnValue(true),
					isFile: vi.fn().mockReturnValue(false),
				} as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.acode/rules/regular.txt") {
				return Promise.resolve("regular file content")
			}
			if (normalizedPath === "/fake/path/.acode/symlink-target.txt") {
				return Promise.resolve("symlink target content")
			}
			if (normalizedPath === "/fake/path/.acode/rules/symlink-target-dir/subdir_link.txt") {
				return Promise.resolve("regular file content under symlink target dir")
			}
			if (normalizedPath === "/fake/path/.acode/nested-symlink-target.txt") {
		...
		const expectedRegularPath =
			process.platform === "win32"
				? "\\fake\\path\\.acode\\rules\\regular.txt"
				: "/fake/path/.acode/rules/regular.txt"
		const expectedSymlinkPath =
			process.platform === "win32"
				? "\\fake\\path\\.acode\\symlink-target.txt"
				: "/fake/path/.acode/symlink-target.txt"
		const expectedSubdirPath =
			process.platform === "win32"
				? "\\fake\\path\\.acode\\rules\\symlink-target-dir\\subdir_link.txt"
				: "/fake/path/.acode/rules/symlink-target-dir/subdir_link.txt"
		const expectedNestedPath =
			process.platform === "win32"
				? "\\fake\\path\\.acode\\nested-symlink-target.txt"
				: "/fake/path/.acode/nested-symlink-target.txt"
		...
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.acode/rules/link.txt")
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.acode/rules/link_dir")

		// Verify both files were read
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.acode/rules/regular.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.acode/symlink-target.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.acode/rules/symlink-target-dir/subdir_link.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.acode/nested-symlink-target.txt", "utf-8")
	})
	beforeEach(() => {
		vi.clearAllMocks()
	})
	...
	it.skipIf(process.platform === "win32")("should correctly format multiple files from directory", async () => {
		...
		// Simulate listing files
		readdirMock.mockResolvedValueOnce([
			{ name: "file1.txt", isFile: () => true, parentPath: "/fake/path/.acode/rules" },
			{ name: "file2.txt", isFile: () => true, parentPath: "/fake/path/.acode/rules" },
			{ name: "file3.txt", isFile: () => true, parentPath: "/fake/path/.acode/rules" },
		] as any)
		...
		statMock.mockImplementation((path) => {
			...
			expect([
				"/fake/path/.acode/rules/file1.txt",
				"/fake/path/.acode/rules/file2.txt",
				"/fake/path/.acode/rules/file3.txt",
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.acode/rules/file3.txt") {
	...
	it("should return files in alphabetical order by filename", async () => {
		...
		readdirMock.mockResolvedValueOnce([
			{ name: "zebra.txt", isFile: () => true, parentPath: "/fake/path/.acode/rules" },
			{ name: "alpha.txt", isFile: () => true, parentPath: "/fake/path/.acode/rules" },
			{ name: "Beta.txt", isFile: () => true, parentPath: "/fake/path/.acode/rules" }, // Test case-insensitive sorting
		] as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			const pathStr = filePath.toString()
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.acode/rules/zebra.txt") {
				return Promise.resolve("zebra content")
			}
			if (normalizedPath === "/fake/path/.acode/rules/alpha.txt") {
				return Promise.resolve("alpha content")
			}
			if (normalizedPath === "/fake/path/.acode/rules/Beta.txt") {
		...
		const expectedAlphaPath =
			process.platform === "win32"
				? "\\fake\\path\\.acode\\rules\\alpha.txt"
				: "/fake/path/.acode/rules/alpha.txt"
		const expectedBetaPath =
			process.platform === "win32" ? "\\fake\\path\\.acode\\rules\\Beta.txt" : "/fake/path/.acode/rules/Beta.txt"
		const expectedZebraPath =
			process.platform === "win32"
				? "\\fake\\path\\.acode\\rules\\zebra.txt"
				: "/fake/path/.acode/rules/zebra.txt"
	...
	it("should sort symlinks by their symlink names, not target names", async () => {
		...
		// First call: check if .acode/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
