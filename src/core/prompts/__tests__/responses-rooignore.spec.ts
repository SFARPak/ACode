...
describe("RooIgnore Response Formatting", () => {
	...
	describe("formatResponse.acodeIgnoreError", () => {
		...
		it("should format error message for ignored files", () => {
			...
			expect(errorMessage).toContain(
				"Access to secrets/api-keys.json is blocked by the .acodeignore file settings",
			)
			expect(errorMessage).toContain("continue in the task without using this file")
			expect(errorMessage).toContain("ask the user to update the .acodeignore file")
		})
	...
	describe("getInstructions", () => {
		 ...
		 */
		it("should format .acodeignore instructions for the LLM", async () => {
			// Create controller
			const controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
		...
		it("should return undefined when no .acodeignore exists", async () => {
			// Set up no .acodeignore
			mockFileExists.mockResolvedValue(false)

			// Create controller without .acodeignore
			const controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
