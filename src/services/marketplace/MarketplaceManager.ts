...
import type { OrganizationSettings, MarketplaceItem, MarketplaceItemType, McpMarketplaceItem } from "@acode/types"
...
export class MarketplaceManager {
	...
	private async checkProjectInstallations(metadata: Record<string, { type: string }>): Promise<void> {
		try {
			...
			// Check modes in .acodemodes
			const projectModesPath = path.join(workspaceFolder.uri.fsPath, ".acodemodes")
			try {
			...
			// Check MCPs in .acode/mcp.json
			const projectMcpPath = path.join(workspaceFolder.uri.fsPath, ".acode", "mcp.json")
			try {
