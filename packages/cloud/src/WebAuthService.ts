...
import { getClerkBaseUrl, getACodeApiUrl, PRODUCTION_CLERK_BASE_URL } from "./config.js"
...
export class WebAuthService extends EventEmitter<AuthServiceEvents> implements AuthService {
	...
	public async login(): Promise<void> {
		try {
			...
			const url = `${getACodeApiUrl()}/extension/sign-in?${params.toString()}`
			await vscode.env.openExternal(vscode.Uri.parse(url))
		} catch (error) {
			this.log(`[auth] Error initiating ACode Cloud auth: ${error}`)
			throw new Error(`Failed to initiate ACode Cloud authentication: ${error}`)
		}
	}

	/**
	 * Handle the callback from ACode Cloud
	 *
	 * This method is called when the user is redirected back to the extension
	 * after authenticating with ACode Cloud.
	 *
	...
	public async handleCallback(
		...
		if (!code || !state) {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Invalid ACode Cloud sign in url")
			}
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Successfully authenticated with ACode Cloud")
			}

			this.log("[auth] Successfully authenticated with ACode Cloud")
		} catch (error) {
			this.log(`[auth] Error handling ACode Cloud callback: ${error}`)
			this.changeState("logged-out")
			throw new Error(`Failed to handle ACode Cloud callback: ${error}`)
		}
	}
	...
	public async logout(): Promise<void> {
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Logged out from ACode Cloud")
			}

			this.log("[auth] Logged out from ACode Cloud")
		} catch (error) {
			this.log(`[auth] Error logging out from ACode Cloud: ${error}`)
			throw new Error(`Failed to log out from ACode Cloud: ${error}`)
		}
	}
